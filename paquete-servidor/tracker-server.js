...
const MOBILE_HTML_FILE = path.join(__dirname, 'mobile-master.html');
const MOBILE_MANIFEST_FILE = path.join(__dirname, 'mobile-master.webmanifest');
const MOBILE_SW_FILE = path.join(__dirname, 'mobile-master-sw.js');
const MOBILE_ICON_FILE = path.join(__dirname, 'mobile-icon.svg');
...
function sendFile(res, filePath, contentType, extraHeaders) {
    try {
        ...
        sendText(res, 200, body, contentType, extraHeaders);
    } catch (error) {
        sendJson(res, 500, { error: 'asset not available' });
    }
}
...
function removeTargetCoord(playerKeyRaw, coordRaw) {
    const playerKey = normalizeText(playerKeyRaw);
    const coord = normalizeCoord(coordRaw);
    if (!playerKey || !coord) {
        throw new Error('playerKey y coord requeridos');
    ...
    const target = getTargetOrThrow(playerKey);
    target.coords = uniqueCoords((target.coords || []).filter((item) => item !== coord));
    target.updatedAt = nowTs();
    state.targets[playerKey] = target;
    rebuildPlanIfNeeded(playerKey);
    markUpdated();
...
function sendPublicAsset(reqUrl, res) {
    if (reqUrl.pathname === API_BASE + '/mobile/') {
        sendRedirect(res, API_BASE + '/mobile');
        return true;
    }
    if (reqUrl.pathname === API_BASE + '/mobile') {
        sendFile(res, MOBILE_HTML_FILE, 'text/html; charset=utf-8');
        return true;
    }
    if (reqUrl.pathname === API_BASE + '/mobile.webmanifest') {
        sendFile(res, MOBILE_MANIFEST_FILE, 'application/manifest+json; charset=utf-8');
        return true;
    }
    if (reqUrl.pathname === API_BASE + '/mobile-sw.js') {
        sendFile(res, MOBILE_SW_FILE, 'text/javascript; charset=utf-8', {
            'service-worker-allowed': API_BASE + '/'
        });
        return true;
    }
    if (reqUrl.pathname === API_BASE + '/mobile-icon.svg') {
        sendFile(res, MOBILE_ICON_FILE, 'image/svg+xml; charset=utf-8');
        return true;
    }
    return false;
}
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && sendPublicAsset(reqUrl, res)) {
        return;
    }
    ...
    try {
        ...
        if (reqUrl.pathname === API_BASE + '/targets/coords/remove') {
            const target = removeTargetCoord(body && body.playerKey, body && body.coord);
            sendJson(res, 200, { ok: true, target: target, state: buildStatePayload() });
            return;
        }
